...
async def create_challenge(wallet_address: str = Query(...)):
    """Create a signing challenge for WalletConnect auth (POST)."""
...
async def get_challenge(wallet_address: str = Query(...)):
    """Create a signing challenge for WalletConnect auth (GET).
    Mirrors the POST endpoint so frontend GET requests work.
    """
    nonce = secrets.token_hex(16)
    message = (
        f"ChainWatch Authentication\n\n"
